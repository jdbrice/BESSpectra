...
Int_t StRefMultCorrEventQAMaker::Init(){
	...
	if ( cfg->exists( "histograms") ){
		...
		ERROR( "Could not make histograms " );
	...
	INFO( "Found " << triggersToSelect.size() << " triggers" );
	for ( int i = 0; i < triggersToSelect.size(); i++ ){
		INFO( "Triggering on " << triggersToSelect[ i ] );
	}
	...
	*/
	...
	for ( int i = 0; i < nPNames.size(); i++ ){
		...
		for ( int j = 0; j < histos.size(); j++ ){
			...
			string newHName = (name + "_" + hName);
			if ( !book->exists( newHName ) ){
				INFO( "Cloning " << hName << " to " << (name + "_" + hName) );
				book->clone( hName, newHName );
			}		
	...
	INFO( "Event Cuts : " );
	INFO( "\tcutVertexZ : " << cutVertexZ->toString() );
	INFO( "\tcutVertexR : " << cutVertexR->toString() );
	INFO( "\tcutVertexROffset : " << vertexROffset->toString() );
	INFO( "\tcutTofMult : " << cutTofMatches->toString() );
